// Summary: 256 state stipple & changed stipple channel + fixed invisible lamp glass at day + improved window crossfade thanks to RaphaelK12 + added lamppost on/off switch
// Custom registers: c223
// Generated by Microsoft (R) HLSL Shader Compiler 9.26.952.2844
//
// Parameters:
//
//   sampler2D StippleTexture;
//   sampler2D TextureSampler;
//   float4 colorize;
//   float emissiveMultiplier;
//   float4 gDayNightEffects;
//   float4 globalScalars;
//
//
// Registers:
//
//   Name               Reg   Size
//   ------------------ ----- ----
//   globalScalars      c39      1
//   gDayNightEffects   c45      1
//   colorize           c51      1
//   emissiveMultiplier c66      1
//   TextureSampler     s0       1
//   StippleTexture     s10      1
//

    ps_3_0
    def c219, 1.8395173895e+25, 3.9938258725e+24, 4.5435787456e+30, 2.3541814201e-43 // 168
    def c150, 15.996, 16, 0.0625, 0.0625	// 256 state stipple constants
    def c127, 0.9999999, 1, 0, 0	// LogDepth constants
    def c0, 3.99600005, 4, 0.125, 0.25
    def c1, 0, -1, -0, 0.00039
    def c2, 0, 0.001, 0.999, 1
    def c3, 5, 3, 0.00390625, 0
    dcl_texcoord v0.xy
    dcl_color v1
    dcl vPos.xy
    dcl_texcoord9 v9
    dcl_2d s0
    dcl_2d s10
	mov r5, c39
	add r5.xy, r5.xy, -c2.zy
	add r6.xy, vPos.xy, -c3.xy
	cmp r6.xy, r5.y, vPos.xy, r6.xy
    mov_sat r0.x, c39.x
    mul r0.x, r0.x, c150.y
    frc r0.y, r0.x
    mul r0.z, r0.y, c150.y
    frc r0.w, r0.z
    add r1.xy, r0.zxzw, -r0.wyzw
    mul r0.xy, c150.z, r6
    frc r0.xy, r0_abs
    cmp r0.xy, r6, r0, -r0
    mul r0.xy, r0, c150.w
    mad r0.xy, r1, c150.w, r0
    mov r0.zw, c1.x
	add r1.xy, c2.w, -r0.xy
	add r1.xy, r1.xy, -c3.z
	cmp r0.xy, r5.y, r0.xy, r1.xy
    texldl r0, r0, s10
	add r1, c2.w, -r0
	cmp r0, r5.y, r0, r1
	add r1, c2.w, -r0
	cmp r0, r5.x, r1, r0
    cmp r0, -r0.z, c1.y, c1.z
    texkill r0
    texld r0, v0, s0
    mul r0, r0, v1
    mov r1.x, c66.x
    mul r1.x, r1.x, c45.w
    mul r1.xyz, r0, r1.x
	// ---------------------------------------------------------- Lamppost On/Off Switch ------------------------------------------------------------
	mov r20.z, -c1.y
	abs r20.w, v9.x
	add r20.w, c1.w, -r20.w
	cmp r20.w, -r20_abs.w, c223.w, r20.z // checks if model ZShift matches c1.w or -c1.w
	mul r0.w, r0.w, r20.w
	// ----------------------------------------------------------------------------------------------------------------------------------------------
    // mul r1.w, r0.w, c39.x
	mov r1.w, r0.w
    mul oC0, r1, c51
	// ----------------------------------------------------------------- Linear2Log -----------------------------------------------------------------
	if_ne v9.y, c127.y
		rcp r20.z, c128.x
		min r20.w, v9.w, c128.y
		mul r20.x, r20.w, r20.z
		mul r20.y, c128.y, r20.z
		log r20.x, r20.x
		log r20.y, r20.y
		rcp r20.y, r20.y
		mad r20.z, r20.x, r20.y, -v9.x
	else
		mov r20.x, v9.z
		rcp r20.y, v9.w
		mul r20.z, r20.x, r20.y
	endif
	mov oDepth, r20.z
	// ----------------------------------------------------------------------------------------------------------------------------------------------

// approximately 23 instruction slots used (3 texture, 20 arithmetic)
